    ...
    >
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                Physical keyboard commands for directors and master control operators. Press <kbd className="px-1.5 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-[10px] text-sky-300">?</kbd> anytime to toggle.
              </p>
            </div>
          </div>
          <button
            ...
            <div key={idx} className="space-y-2.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {group.shortcuts.map((sc, scIdx) => (
                  <div 
                    key={scIdx}
                    className="p-2.5 rounded-xl bg-slate-900/80 border border-slate-800/80 flex items-center justify-between gap-3 hover:border-slate-700 transition"
                  >
                    <span className="text-xs text-slate-300 font-medium">
                      {sc.desc}
                    </span>
                    <kbd className="px-2 py-1 bg-slate-950 border border-slate-700 rounded text-[11px] font-mono font-bold text-sky-400 shadow-inner shrink-0">
                      {sc.key}
                    </kbd>
                  </div>
                ))}
        ...
        <div className="flex items-center justify-between px-6 py-3 border-t border-slate-800/80 bg-slate-900/40 text-xs text-slate-400">
          <span>Standard US/ISO Keyboard Map</span>
          <button
            onClick={onClose}
